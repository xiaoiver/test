$(document).ready(function(){
	...
	function addOnClickFunc(currentId, id, name, props){
		$("#"+currentId).click(function(){
			...
			$("#editDocumentForm").attr("action","document/editDocument.action?documentId="+documentId+"&documentTypeId="+id+"");//通过form的action传文献类型id
			...
			for(var i=0; i<allTypes.length; i++){
				...
				var currentVal = $("#hid-"+currentType[0]).val();
				$("#buttons").before("<div class='addedInput input-group'>"
						+"<input type='text' name='"+currentType[0]+"' id='"+currentType[0]+"' value='"+currentVal+"' class='form-control' data-required />"
	...
	$.ajax({
		...
		$("#dt-"+documentTypeId+"-Link").click();//触发当前文献类型链接的onclick事件
	});
